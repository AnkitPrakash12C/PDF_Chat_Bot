...
class PDFChatAssistant:
    ...
    def load_and_index_pdf(self):
        ...
        start = time.time()
        st.write("Embedding all chunks... ")
        # Note: SentenceTransformer is local, so this part remains the same.
        model = SentenceTransformer('multi-qa-MiniLM-L6-cos-v1')
    ...
    def query(self, question, preview=True):
        ...
        # --- 4) call LLM (Gemini) ---
        try:
            # Use the globally initialized Gemini model
            ...
            print("⚠️ API Error:", e)
            # Fallback error message. In a real app, you might want to log 'e' more robustly.
            answer_text = f"Sorry, I couldn't generate an answer due to an API error: {e}"
